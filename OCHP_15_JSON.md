...
```shell
# You can also use wget
curl -X GET /ChargePointList \
  -H 'Accept: application/json'

```

```http
GET /ChargePointList HTTP/1.1
...
result = RestClient.get '/ChargePointList',
  params: {
  }, headers: headers
...
r = requests.get('/ChargePointList', headers = headers)
...
try {
    $response = $client->request('GET','/ChargePointList', array(
        'headers' => $headers,
        'json' => $request_body,
       )
    );
...
func main() {
    ...
    data := bytes.NewBuffer([]byte{jsonReq})
    req, err := http.NewRequest("GET", "/ChargePointList", data)
    req.Header = headers
